...
function TimeStamp({ createdDate, updatedDate }) {
  ...
  useEffect(() => {
    ...
    //   result = `${updatedAt.toLocaleDateString()} ${updatedAt.toLocaleTimeString()}`;
    result = `${updatedAt.toLocaleString('default', {month: 'long'})} ${updatedAt.getDay()}`
      setUpdate(result);
    }
  }, []);
  return <>{`Posted: ${monthCreated} ${dayCreated}, ${yearCreated} ${timeCreated} EST`} | {`Last Updated: ${update && update}`}</>;
}
